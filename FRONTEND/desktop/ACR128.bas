...
ConnActive = False
End Sub
Public Sub ConnectACR128()
InitializeACR128
MsgBox ConnActive
  If ConnActive Then
    MsgBox ("Connection is already active.")
    Exit Sub
   End If
  retCode = SCardConnect(hContext, "ACS ACR128U ICC Interface 0", SCARD_SHARE_EXCLUSIVE, SCARD_PROTOCOL_T0 Or SCARD_PROTOCOL_T1, hCard, Protocol)
  If retCode <> SCARD_S_SUCCESS Then
    'Call DisplayOut(1, retCode, "")
    ConnActive = False
    Exit Sub
  Else
    MsgBox "Successful connection to ACS ACR128U ICC Interface 0"
  End If
  MsgBox retCode
  ConnActive = True
End Sub

Public Sub ResetACR128()
  If ConnActive Then
    retCode = SCardDisconnect(hCard, SCARD_UNPOWER_CARD)
    ConnActive = False
  End If
  retCode = SCardReleaseContext(hContext)
  MsgBox ("Reset Success")
End Sub
