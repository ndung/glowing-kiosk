...
public class PaymentPlnProcessor implements Processor {

    @Override
    public ISOMsg prepareRequest(MessageUtil message) throws ISOException {
        String bit48 = "";
        bit48 += message.getBit48().substring(0, 13);
        bit48 += message.getJumlahTagihan();
        bit48 += message.getBit48().substring(13);

        ISOMsg isoMsg = new ISOMsg();

        isoMsg.setMTI("0200");
        isoMsg.set(3, "811000");
        isoMsg.set(4, StringUtil.addLeadingZeroes(message.getPrice(), 12));
