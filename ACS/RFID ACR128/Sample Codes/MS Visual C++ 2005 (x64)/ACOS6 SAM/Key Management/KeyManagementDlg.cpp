...
//
//
// Description : This program implements SAM commands 
//				 All general functions and variables are all declared on
//					on KeyManagementDlg.cpp and KeyManagementDlg.h
//					for public usage.
//
...
// KeyManagementDlg.cpp : implementation file
//
...
BOOL CKeyManagementDlg::OnInitDialog()
{
	...
	// TODO: Add extra initialization here
	 ...
	 TCITEM tcItem2;
	   ...
	   tcItem2.pszText = _T("        ACOS Card Initialitaion      ");
	...
	this->SetWindowText("Key Management Sample");
...
void CKeyManagementDlg::OnSelchangeTab2(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
{	
    ...
    for (indx = 0;indx<255;indx++)
    {
		...
		G_SendBuff[indx] = 0x00;
		G_RecvBuff[indx] = 0x00;
...
int CKeyManagementDlg::DisplayOut(int errType, int retVal, CString PrintText, CString AppText)
{	
...
int CKeyManagementDlg::AppendSamFile(BYTE KeyId, BYTE* DataArr, int MaxDataLen)
{	
...
int CKeyManagementDlg::CreateSAMFile(BYTE FileLen, BYTE* DataArr, int MaxDataLen)
{	
...
int CKeyManagementDlg::writeRecord(int caseType, BYTE RecNo, BYTE maxDataLen, BYTE DataLen, BYTE* DataIn)
...
int CKeyManagementDlg::SendAPDUandDisplay(int SendType, CString ApduIN) 
{	
...
CString CKeyManagementDlg::GetSAMResponse()
...
int CKeyManagementDlg::GenerateSAMKey(BYTE KeyId, BYTE* DataArr, int MaxDataLen) 
...
int CKeyManagementDlg::SendAPDUSAM(BYTE* SendBuff, int SendLen, int RecvLen, BYTE* RecvBuff)
{	
	//Sends the APDU command to the SAM Card...
	//returns 1 if erronous and 0 if successful
...
void CKeyManagementDlg::ResetSAM()
{
	//Resets the SAM Card..
...
void CKeyManagementDlg::ResetMCU()
{
	//Resets the MCU Card..
...
int CKeyManagementDlg::readRecord(BYTE RecNo, BYTE DataLen)
{	
...
int CKeyManagementDlg::selectfile(BYTE HiAddr, BYTE LoAddr) 
{	
...
void CKeyManagementDlg::SubmitIC() 
{
