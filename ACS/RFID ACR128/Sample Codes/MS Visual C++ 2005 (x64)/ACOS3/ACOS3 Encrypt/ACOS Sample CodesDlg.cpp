...
void CACOSSampleCodesDlg::OnButton4() 
{
	...
	{	
		...
		SendBuff[3] = 0x00; //P2
		SendBuff[4] = 0x10; //P3
		SendBuff[5] = tmpArray1[0];
		SendBuff[6] = tmpArray1[1];
		SendBuff[7] = tmpArray1[2];
		SendBuff[8] = tmpArray1[3];
		SendBuff[9] = tmpArray1[4];
		SendBuff[10] = tmpArray1[5];
		SendBuff[11] = tmpArray1[6];
		SendBuff[12] = tmpArray1[7];
		SendBuff[13] = tmpArray1[8];
		SendBuff[14] = tmpArray1[9];
		SendBuff[15] = tmpArray1[10];
		SendBuff[16] = tmpArray1[11];
		SendBuff[17] = tmpArray1[12];
		SendBuff[18] = tmpArray1[13];
		SendBuff[19] = tmpArray1[14];
		SendBuff[20] = tmpArray1[15];
		...
		if (m_radio3.GetCheck() == 1)
		{
			...
			for (indx = 0;indx<8;indx++)
			{
				tmpArray1[indx] = tmpArray1[indx] ^= TRnd[indx];
			}	
			...
			for (indx = 0;indx<8;indx++)
			{
				SessionKey[indx] = tmpArray1[indx]; 
			}	
		...
		else
		{
			...
			for (indx = 0;indx<8;indx++)
			{
				tmpArray1[indx] = TRnd[indx];
			}	
			DES3 (tmpArray1, ReverseKey);
			
			//Right Half of Session Key is done
			for (indx = 0;indx<8;indx++)
			{
				SessionKey[indx + 8] = tmpArray1[indx];
			}	
	...
	if  (this->m_combo4.GetCurSel() == 0)
	{
		tmpbyte = 0x06;        // Record No
		strcpy(buff1,"PIN");
	}
	else
	if  (this->m_combo4.GetCurSel() == 1)
	{
		tmpbyte = 0x01;        // Record No
		strcpy(buff1,"Application Code 1");
	}
	else
	if  (this->m_combo4.GetCurSel() == 2)
	{
		tmpbyte = 0x02;        // Record No
		strcpy(buff1,"Application Code 2");
	}
	else
	if  (this->m_combo4.GetCurSel() == 3)
	{
		tmpbyte = 0x03;        // Record No
		strcpy(buff1,"Application Code 3");
	}
	else
	if  (this->m_combo4.GetCurSel() == 4)
	{
		tmpbyte = 0x04;        // Record No
		strcpy(buff1,"Application Code 4");
	}
	else
	if  (this->m_combo4.GetCurSel() == 5)
	{
		tmpbyte = 0x05;        // Record No
		strcpy(buff1,"Application Code 5");
	}

		//Issue Submit Code Command
		RecvLength = 0x00;
		SendBuff[0] = 0x80;        // CLA
		SendBuff[1] = 0x20;        // INS
		SendBuff[2] = tmpbyte;        // Record No
		SendBuff[3] = 0x00;        // P2
		SendBuff[4] = 0x08;        // P3
		...
		if (RetCode != SCARD_S_SUCCESS)
		{
			i = m_ListBox.AddString ("SUBMIT CODE command failed!");
			m_ListBox.SetCurSel (i);
		...
		sprintf(buff1, "%s %s", buff1, " submitted successfully!");
...
void CACOSSampleCodesDlg::OnButton7() 
{
	char buff[100];
	char buff1[100];
	int i,ctr;
	...
	if  (this->m_combo4.GetCurSel() == 4)
	{
		SendBuff[2] = 0x08;        // Record No
		strcpy(buff1,"Application Code 4");
	}
	else
	if  (this->m_combo4.GetCurSel() == 5)
	{
		SendBuff[2] = 0x09;        // Record No
		strcpy(buff1,"Application Code 5");
	}
	...
	i = 5;
		...
		if (RetCode != SCARD_S_SUCCESS)
		{
			i = m_ListBox.AddString ("Error in Updating Code!");
			m_ListBox.SetCurSel (i);
		...
		sprintf(buff1, "%s %s", buff1, " changed successfully!");
		i = m_ListBox.AddString (buff1);
		m_ListBox.SetCurSel (i);
		sprintf(buff, "%02X %02X", RecvBuff[0],RecvBuff[1]);
		i = m_ListBox.AddString (buff);
