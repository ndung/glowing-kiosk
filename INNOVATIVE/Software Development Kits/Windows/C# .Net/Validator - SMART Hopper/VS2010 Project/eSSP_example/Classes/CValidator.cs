...
namespace eSSP_example
{
    public class CValidator
    {
        ...
        // Buffer to hold the poll response, storing the poll response in this buffer avoids losing data
        // if the command structure is used again before the full poll packet is parsed.
        ...
        public void Reset(TextBox log = null)
        {
            ...
            if(CheckGenericResponses(log))
                log.AppendText("Resetting Validator\r\n");
        }
        ...
        {
            cmd.CommandData[0] = CCommands.Validator.SSP_CMD_LAST_REJECT_CODE;
            cmd.CommandDataLength = 1;
            if (!SendCommand(log)) return;
