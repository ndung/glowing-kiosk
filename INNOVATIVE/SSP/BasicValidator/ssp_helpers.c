...
SSP_RESPONSE_ENUM ssp6_payout(SSP_COMMAND *sspC, const int value, const char *cc, const char option)
{
    ...
    int l;
    for (l=0;l<sspC->CommandDataLength;l++){
        printf("data %02X\n",sspC->CommandData[l]);
    }
...
SSP_RESPONSE_ENUM ssp6_payout_by_denom(SSP_COMMAND *sspC, const char *data, const char *cc, const char option)
{
    SSP_RESPONSE_ENUM resp;

    char denoms_to_payout[2];
    strncpy(denoms_to_payout,&data[0],1);
    denoms_to_payout[1] = '\0';
    int num = atoi(denoms_to_payout);

    sspC->CommandDataLength = 9*num+3;
    sspC->CommandData[0] = 0x46;
    sspC->CommandData[1] = num >> 0;

    int i;
    for (i=0;i<num;i++){
        char num_to_payout_char[13];
        strncpy(num_to_payout_char,&data[24*i+1],12);
        num_to_payout_char[12] = '\0';

        char denom_value_char[13];
        strncpy(denom_value_char,&data[24*i+13],12);
        denom_value_char[12] = '\0';

        int num_to_payout = atoi(num_to_payout_char);
        int denom_value = atoi(denom_value_char)*100;

        int j;

        for (j=0; j<2; j++){
            int index = i*9+j+2;
            sspC->CommandData[index] = num_to_payout >> (j*8);
        }

        for (j=0; j<4; j++){
            int index = i*9+j+4;
            sspC->CommandData[index] = denom_value >> (j*8);
        }

        for (j=0; j<3; j++){
            int index = i*9+j+8;
            sspC->CommandData[index] = cc[j];
        }
    }
    sspC->CommandData[9*num+2] = option;
    resp = _ssp_return_values(sspC);
...
SSP_RESPONSE_ENUM ssp6_get_route(SSP_COMMAND *sspC, const int value, const char *cc){
    SSP_RESPONSE_ENUM resp;
    int i;

    sspC->CommandDataLength = 8;
    sspC->CommandData[0] = 0x3C;
...
// smart empty
SSP_RESPONSE_ENUM ssp6_smart_empty(SSP_COMMAND *sspC)
{
    SSP_RESPONSE_ENUM resp;
...
SSP_RESPONSE_ENUM ssp6_get_cashbox_payout_op_data(SSP_COMMAND *sspC){
    SSP_RESPONSE_ENUM resp;

    sspC->CommandDataLength = 1;
    sspC->CommandData[0] = 0x53;

    resp = _ssp_return_values(sspC);
    return resp;
}

SSP_RESPONSE_ENUM ssp6_query_rejection(SSP_COMMAND *sspC){
    SSP_RESPONSE_ENUM resp;
...
// check note level
SSP_RESPONSE_ENUM ssp6_check_note_level(SSP_COMMAND *sspC, const int value, const char *cc)
{
    SSP_RESPONSE_ENUM resp;
    int i;
...
SSP_RESPONSE_ENUM ssp6_float(SSP_COMMAND *sspC, const int min, const int value, const char *cc, const char option)
{
    SSP_RESPONSE_ENUM resp;
    int i;

    sspC->CommandDataLength = 13;
    sspC->CommandData[0] = SSP_CMD_FLOAT_AMOUNT;
    
    for (i=0; i<4; i++)
	sspC->CommandData[i+1] = min >> (i*8);
    
    for (i=0; i<4; i++)
	sspC->CommandData[i+5] = value >> (i*8);
		
    for (i=0; i<3; i++)
	sspC->CommandData[i+9] = cc[i];

    sspC->CommandData[12] = option;
