...
namespace eSSP_example
{
    public class CNV11
    {
        ...
        {
            cmd.CommandData[0] = CCommands.SSP_CMD_PAYOUT_LAST_NOTE;
            cmd.CommandDataLength = 1;
            if (!SendCommand(log)) return;
        ...
        {
            cmd.CommandData[0] = CCommands.SSP_CMD_SET_VALUE_REPORTING_TYPE;
            if (byChannel)
        ...
        {
            cmd.CommandData[0] = CCommands.SSP_CMD_STACK_LAST_NOTE;
            cmd.CommandDataLength = 1;
            if (!SendCommand(log)) return;
        ...
        public string GetStorageInfo()
        {
            ...
            for (int i = 0; i < m_NotePositionValues.Length; i++)
            {
                if (m_NotePositionValues[i] > 0)
                    s += "Position " + i + ": " + m_NotePositionValues[i] / 100 + "\r\n";
            }
            return s;
        }
