#!/bin/sh
#
# This is a driver installer for TouchKit controller.
#
...
CheckOldDaemon() {
    ...
    if [ $? -eq 0 ]; then
        echo "(W) Found the old TouchKit daemon: tpaneld."
    ...
    if [ $? -eq 0 ]; then
        echo "(W) Found the old TouchKit daemon: usbpnpd."
        echo "(E) It has to be removed before run this installer."
        echo ""
        rm -f ${processfile}
        exit 1
...
CheckOldDriver() {
    ...
    if [ -z "${olddriverpath}" ]; then
        CheckOldDaemon
        echo "(I) Begin to setup the TouchKit driver."
    else
        echo "(W) Found the old TouchKit driver."
        echo "(E) It has to be removed before run this installer."
        ${olddriver}
    fi
}
...
ExtractDriver() {
    ...
    if [ -n "${driverpath}" ]; then
        rm -rf ${driverpath}
        echo "(I) Found and removed previous TouchKit driver."
    fi
    ...
    temppath=`find ${installpath} -name ${utility}`
    driverpath=${temppath%/*}
    echo "(I) Extract TouchKit driver archive to ${driverpath}."
...
RemoveDriver() {
    ...
    if [ -z "${driverpath}" ]; then
        ...
        rm -rf ${driverpath}
        echo "(I) Removed TouchKit driver archive from ${driverpath}."
    fi
...
Add2Blacklist() {
    if [ -w ${blacklistpath} ]; then
        ...
        if [ $? -eq 1 ]; then
            ...
            sed -i ''${filelines}'a\# added by TouchKit installer\nblacklist '$1'' ${blacklistpath}
        else
...
RestoreBlacklist() {
    if [ -w ${blacklistpath} ]; then
        ...
        if [ $? -eq 0 ]; then
            grep -q "# added by TouchKit installer" ${blacklistpath}
            if [ $? -eq 0 ]; then
                sed -i '/# added by TouchKit installer/,/blacklist usbtouchscreen/d' ${blacklistpath}
            ...
            if [ $? -eq 0 ]; then
                grep -q "# added by TouchKit installer" ${blacklistpath}
                if [ $? -eq 0 ]; then
                    sed -i '/# added by TouchKit installer/,/blacklist touchkitusb/d' ${blacklistpath}
        ...
        if [ $? -eq 0 ]; then
            grep -q "# added by TouchKit installer" ${blacklistpath2}
            if [ $? -eq 0 ]; then
                sed -i '/# added by TouchKit installer/,/blacklist usbtouchscreen/d' ${blacklistpath2}
            ...
            if [ $? -eq 0 ]; then
                grep -q "# added by TouchKit installer" ${blacklistpath2}
                if [ $? -eq 0 ]; then
                    sed -i '/# added by TouchKit installer/,/blacklist touchkitusb/d' ${blacklistpath2}
        ...
        if [ $? -eq 0 ]; then
            grep -q "# added by TouchKit installer" ${blacklistpath3}
            if [ $? -eq 0 ]; then
                sed -i '/# added by TouchKit installer/,/usbtouchscreen/d' ${blacklistpath3}
            ...
            if [ $? -eq 0 ]; then
                grep -q "# added by TouchKit installer" ${blacklistpath3}
                if [ $? -eq 0 ]; then
                    sed -i '/# added by TouchKit installer/,/touchkitusb/d' ${blacklistpath3}
        ...
        if [ $? -eq 0 ]; then
            grep -q "# added by TouchKit installer" ${rclocalpath}
            if [ $? -eq 0 ]; then
                sed -i '/# added by TouchKit installer/,/rmmod usbtouchscreen/d' ${rclocalpath}
            ...
            if [ $? -eq 0 ]; then
                grep -q "# added by TouchKit installer" ${rclocalpath}
                if [ $? -eq 0 ]; then
                    sed -i '/# added by TouchKit installer/,/rmmod touchkitusb/d' ${rclocalpath}
...
if [ $# = 0 ]; then
    ...
    if [ $1 = "uninstall" ]; then
        CheckPermission
        echo "(I) Begin to remove TouchKit driver."
        RemoveDriver
        RemoveXorgModule
        RestoreBlacklist
        RestoreConfigFile
